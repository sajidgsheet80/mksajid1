...
# ===== Configuration =====
# IMPORTANT: Replace this with your actual mStock API Secret
...
def login_mstock():
    ...
    if not creds or not creds['mstock_api_key']:
        return jsonify({
            "status": "error",
            "message": "mStock API key not configured."
        }), 400
...
# ===== Manual Order Placement Routes =====

@app.route("/place_order", methods=["POST"])
@login_required
def place_manual_order():
    """Handles Manual Order Placement via mStock"""
    username = session['username']
    user_sess = get_user_session(username)
    creds = get_user_credentials(username)
    access_token = user_sess.get('mstock_access_token')
    ...
    transaction_type = data.get('side', 'BUY').upper() # BUY or SELL
    quantity = int(data.get('quantity', 1))
    order_type = data.get('order_type', 'MARKET').upper() # MARKET or LIMIT
    price = float(data.get('price', 0))
    exchange = data.get('exchange', 'NSE').upper()
    product = data.get('product', 'MIS').upper() # MIS (Intraday) or NRML (Delivery)

    if not symbol:
        return jsonify({"status": "error", "message": "Symbol is required"}), 400

    # Prepare mStock Order Payload
    payload = {
    ...
    try:
        ...
        if resp_json.get("status") == "success":
            ...
            # Try to extract more detailed error if available in response structure
...
<body>
    <div class="card">
        ...
        <p style="color:#666; font-size:14px; margin-bottom:20px;">
            Enter your mStock API Key. The API Secret is hardcoded in the server configuration.
        </p>
        <form method="POST">
...
<body>
            ...
            <div id="auth-section">
                ...
                <div id="otp-area" class="otp-form" style="display: none;">
                    ...
                    <div style="display: flex; gap: 10px;">
                        ...
                        <button class="btn btn-primary" style="width: auto;" onclick="authenticate()">Login</button>
                    </div>
            ...
            <form id="order-form">
                ...
                <div class="form-group">
                    ...
                    <div class="toggle-container">
                        <div class="toggle-option active-buy" id="opt-buy" onclick="setSide('BUY')">BUY</div>
                        <div class="toggle-option" id="opt-sell" onclick="setSide('SELL')">SELL</div>
                    </div>
                ...
                <div class="row">
                    <div class="col">
                        <div class="form-group">
                            ...
                            <select id="order_type" name="order_type" onchange="togglePriceInput()">
                ...
                <!-- CHANGED: type="button" to prevent default form submission behavior causing double requests -->
                <button type="button" id="submit-btn" class="btn btn-buy" onclick="placeOrder(event)">PLACE BUY ORDER</button>
            </form>
        </section>
    ...
    <script>
        let currentSide = 'BUY';
        let isSubmitting = false; // GLOBAL LOCK TO PREVENT DOUBLE ORDERS
        ...
        async function checkAuthStatus() {
            try {
                ...
                if (data.status === 'authenticated') {
                    ...
                    addLog('System', 'Connected to mStock successfully.', 'success');
                } else {
                    dot.classList.remove('active');
                    text.innerText = 'Not Authenticated';
                    text.style.color = 'var(--danger)';
                    otpArea.style.display = 'block';
                    addLog('System', 'Authentication required. Please enter OTP.', 'error');
                }
            } catch (e) {
                console.error(e);
                addLog('System', 'Error checking auth status.', 'error');
            }
        }
        ...
        async function placeOrder(e) {
            ...
            // CHECK 1: If a request is already running, stop immediately.
            if (isSubmitting) {
                console.log("Request already in progress. Ignoring click.");
                return;
            }

            // LOCK the button and state
            isSubmitting = true;
            ...
            try {
                ...
                if (data.status === 'success') {
                    ...
                    addLog('ERROR', `${data.side} ${data.symbol} - ${data.message}`, 'error');
